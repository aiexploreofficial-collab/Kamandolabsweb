...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="bg-[#0A0A0A] text-white min-h-screen flex flex-col md:flex-row">
      {/* Mobile Header Bar */}
      <header className="md:hidden bg-[#0A0A0A] border-b border-neutral-850 p-4 flex items-center justify-between z-30">
            ...
            {links.map((link) => {
              ...
              return (
                ...
                >
                  ...
                  <link.icon className={`w-4 h-4 flex-shrink-0 ${isActive ? "text-red-600" : "text-neutral-500 group-hover:text-neutral-300"}`} />
                  {link.label}
                </Link>
              );
            })}
      ...
      {/* Main Panel Content */}
      <main className="flex-1 p-6 md:p-10 max-w-7xl mx-auto w-full overflow-y-auto bg-[#0A0A0A]">
        {children}
      </main>
    </div>
  );
